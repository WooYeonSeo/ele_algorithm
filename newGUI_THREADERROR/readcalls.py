...
class Readcalls(threading.Thread):
       ...
       while (self.tn < 10000):
           if self.tn < 10000:
               ...
               if self.dialog.timer[2] == registertime.second and self.dialog.timer[1] == registertime.minute and \
                   ...
                   self.tn += 1
                   print("t1  : ", self.t1.isSet(), "readycall length",len(self.elevator_rack[i].ready_calls))
   ...
   def allocate3(self,call):
       ...
       for index in range(0, 4):  # constant.MAX_ELEVATORS
           ...
           if (self.elevator_rack[index].state == constant.IDLE_STATE):
               #일단 후보로 둔다
               keep = index
               if (int(self.elevator_rack[index].floor) == call.departure ):
                   ...
                   if (floor_term < self.idle_term):  # 제일 짧은 층 차이보다 더 층 차이가 짧을 때
                       self.idle_term = floor_term  # 층 차이를 업데이트한다
